  ...
  return (
    <div className="w-full bg-white max-w-sm rounded-lg p-6 border-yellow-400 border-4">
      ...
      <p className="text-purple-500 text-6xl font-bold mb-10">
        {details.isFree ? <span>Free</span> :<span>₹{details.priceInRupees}</span>}
        {details.priceSuffix && <span className ="text-lg m1">{details.priceSuffix}</span>}
      </p>
      <ul>
        {details.description.split('*').map((bullet, index) =>(
          <li key={index} className="text-yellow-400 my-2 text-sm">{bullet}</li>
        ))}
      </ul>
      <button className="mx-auto w-full block bg-yellow-400 px-4 py-2 rounded-md text-white mt-6 mb-4">{details.buttonText}</button>
      {details.finePrint && <small className="block text-center text-xm">{details.finePrint}</small>}
    </div>
  )}
  else {
    return (
      <div className="w-full bg-purple-500 max-w-sm rounded-lg p-6">
        ...
        <ul>
          {details.description.split('*').map((bullet, index) =>(
            <li key={index} className="text-purple-100 my-2 text-sm">{bullet}</li>
          ))}
        </ul>
